# Scenario map Mermaid template

Использовать как дополнительную визуализацию к `Delivery readiness pack`, когда команде полезно увидеть путь пользователя или общий flow процесса.

Файл сохраняется как `team/scenario-map.mmd`.

## Вариант 1: flowchart

```mermaid
flowchart TD
  Start([Начало])
  ActorAction["<Роль>: <действие>"]
  Decision{"<Условие или решение>"}
  Success([<Успешное завершение>])
  Alternative["<Альтернативный путь>"]
  Exception["<Исключение или запрет>"]

  Start --> ActorAction
  ActorAction --> Decision
  Decision -->|да| Success
  Decision -->|нет| Alternative
  ActorAction -.-> Exception
```

## Вариант 2: journey

```mermaid
journey
  title <Название сценария>
  section <Роль 1>
    <Шаг>: 3: <Роль 1>
    <Шаг>: 4: <Роль 1>
  section <Роль 2>
    <Шаг>: 3: <Роль 2>
```

## Правила

- Выбирай `flowchart`, если важны ветвления, запреты и альтернативы.
- Выбирай `journey`, если важен путь по ролям и последовательность участия.
- Не добавляй экранов, системных компонентов или технических действий, которых нет в артефактах.
- Каждый узел должен быть понятен человеку без знания ID требований.
- Если шаг является предположением, выноси его в вопросы readiness pack; не изображай его как подтвержденный flow и не добавляй в Mermaid как `candidate`.
