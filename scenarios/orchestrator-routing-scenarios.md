...
| 10 | Файловая фиксация нового прогона | Новый прогон перезапишет старый каталог или разложит артефакты прямо в `runs/` | `artifact-persistence-agent(create_run)` создает отдельный versioned каталог, возвращает `current_run_path` и пишет `Вход` в `current_run_path/product/input.md` | Все update-сохранения используют тот же `current_run_path`; запись в `runs/product`, `runs/service` или `runs/team` считается сбоем фиксации |
...
| 20 | Обязательный служебный агент недоступен | Оркестратор заменит контроль ручной проверкой или исполнит роль в своем основном контексте | Pipeline останавливается, если контрактный файл отсутствует/нечитаем, не определяет нужный I/O, раннер не может создать изолированный запуск сабагента или среда блокирует обязательное действие | Промежуточные артефакты сохраняются как `incomplete`, если возможно |
...
- Если требуется `agents/*.md`, раннер-оркестратор запускает отдельного сабагента; исполнение роли в основном контексте оркестратора является нарушением маршрута.
