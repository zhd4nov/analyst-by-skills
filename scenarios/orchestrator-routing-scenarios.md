...
| 20 | Обязательный служебный агент недоступен | Оркестратор заменит контроль ручной проверкой или ошибочно потребует отдельный runtime-tool при наличии контракта | Pipeline останавливается только если контрактный файл отсутствует/нечитаем, не определяет нужный I/O или среда блокирует обязательное действие; отсутствие отдельного runtime-tool не блокирует вызов | Промежуточные артефакты сохраняются как `incomplete`, если возможно |
