...
- если пользователь согласился на brief, оркестратор обязан затем запустить `delivery-readiness-agent`
- `Delivery readiness pack`, `Scenario map` и применимая `State model diagram` являются обязательным дополнением к заказанному brief
...
Командные post-pipeline файлы:
- `team/kickoff-brief.md` создается только если пользователь явно согласился на подготовку brief после завершения или отказа от интервью.
...
## Сценарий 14: Brief и delivery readiness package формируются после opt-in пользователя
...
8. `artifact-persistence-agent` сохраняет `Kickoff brief` в `team/kickoff-brief.md`
9. Оркестратор вызывает `delivery-readiness-agent` с `delivery_readiness_requested: true`
10. `delivery-readiness-agent` формирует `Delivery readiness pack`, `Scenario map` и применимую `State model diagram`
11. `artifact-persistence-agent` сохраняет командные документы в `team/`
12. Оркестратор возвращает пользователю ссылки на brief, readiness pack и Mermaid-файлы, не подменяя финальные артефакты
...
- `team/state-model.mmd`, если применимо
...
- если пользователь согласился на brief, `delivery-readiness-agent` обязан отработать; pack и диаграммы являются обязательным дополнением к brief
...
- `team/kickoff-brief.md` не создается
- `team/delivery-readiness-pack.md` не создается
...
## Сценарий 18: Командные документы запускаются без отдельного согласия пользователя
...
- Оркестратор пытается вызвать `kickoff-briefing-agent` или `delivery-readiness-agent` как продолжение brief

### Пример входа
- После отказа от интервью система сразу формирует `team/kickoff-brief.md` или `team/delivery-readiness-pack.md`.

### Ожидаемый маршрут
1. Оркестратор считает вызов `kickoff-briefing-agent` недопустимым
2. `kickoff-briefing-agent` и `delivery-readiness-agent` не вызываются
3. Файлы в `team/` не создаются
...
### Ожидаемые артефакты
- `team/kickoff-brief.md` отсутствует
- `team/delivery-readiness-pack.md` отсутствует
- Канонические продуктовые артефакты остаются без изменений

### Ключевое правило
- Командные документы готовятся только после отдельного явного opt-in пользователя на brief или отдельного запроса readiness pack.
...
5. Оркестратор не вызывает `delivery-readiness-agent`, пока brief не исправлен или не принято решение отказаться от командных документов
6. Оркестратор возвращает проблему на корректный этап ядра или останавливается с вопросом пользователю, если без ответа противоречие не закрыть

### Ожидаемые артефакты
- `team/kickoff-brief.md` отсутствует или сохранен как `incomplete`
- `team/delivery-readiness-pack.md` не создается до исправления brief
...
## Сценарий 22: Delivery readiness pack формируется как обязательное дополнение к brief

### Тип входа
- Валидный финальный комплект после успешного ядра и post-pipeline интервью, если оно проводилось

### Предусловие
- Финальный комплект текущего прогона валиден и сохранен
- Если интервью проводилось, затронутые артефакты пересобраны, итоговый аудит `passed`, контроль маршрута `allow`
- Пользователь согласился подготовить `Kickoff brief`
- `Kickoff brief` успешно сформирован и сохранен в `team/kickoff-brief.md`

### Пример входа
- "Да, подготовь brief для команды."

### Ожидаемый маршрут
1. Оркестратор проверяет валидность и сохраненность финального комплекта
2. Оркестратор формирует и сохраняет `Kickoff brief` в `team/kickoff-brief.md`
3. Оркестратор обязательно вызывает `delivery-readiness-agent` с `delivery_readiness_requested: true`
4. Агент формирует `Delivery readiness pack` по `templates/delivery-readiness-template.md`
5. Агент формирует `Scenario map` в Mermaid по `templates/scenario-map-mermaid-template.md`
6. Если в спецификации есть подтвержденные состояния и переходы, агент формирует `State model diagram` в Mermaid по `templates/state-model-mermaid-template.md`
7. Если модель состояний неприменима, агент возвращает `State model diagram: not_applicable` с причиной
8. `artifact-persistence-agent` сохраняет применимые артефакты в `team/`
9. Оркестратор возвращает пользователю ссылки на все командные документы
...
- `team/scenario-map.mmd`
- `team/state-model.mmd`, если применимо
- Канонические продуктовые артефакты не меняются

### Ключевые правила
- Заказанный brief не может быть единственным командным документом; readiness pack и диаграммы обязательны.
- Readiness pack не является backlog, technical breakdown или набором архитектурных решений.
- Candidate work areas для Design, Frontend, Backend и QA должны быть помечены как `candidate`.
- Mermaid-диаграммы являются визуализацией, а не заменой readiness pack.

---

## Сценарий 23: Delivery readiness agent пытается принять технические решения

### Тип входа
- Валидный финальный комплект и запрос на readiness pack

### Предусловие
- Агент выводит API endpoints, payloads, backend-модели, frontend-компоненты или технические задачи как утвержденные решения
- В канонических артефактах нет источника для этих технических решений

### Пример входа
- Агент пишет: "Нужно создать endpoint `POST /api/trips`, таблицу `trip_requests` и компонент `TripRequestForm`" без источника в требованиях или отдельного технического решения команды.

### Ожидаемый маршрут
1. Оркестратор считает результат агента дефектным
2. Оркестратор не сохраняет readiness pack как `complete`
3. Оркестратор возвращает результат `delivery-readiness-agent` на доработку с конкретным замечанием: технические решения должны быть удалены или помечены как `candidate` и вынесены в вопросы команды
4. После исправления агент повторно формирует pack и диаграммы
5. Только исправленный результат сохраняется через `artifact-persistence-agent`

### Ожидаемые артефакты
- До исправления `team/delivery-readiness-pack.md` отсутствует или сохранен как `incomplete`
- В финальном pack нет неподтвержденных технических решений
- Candidate-зоны и вопросы команды явно отделены от требований

### Ключевое правило
- `delivery-readiness-agent` может помогать команде увидеть зоны работы, но не имеет права принимать технические решения за команду.
