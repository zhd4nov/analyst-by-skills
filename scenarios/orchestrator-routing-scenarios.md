...
- `product/delivery-readiness-pack.md` создается только по явному запросу или согласию пользователя после валидного финального комплекта.
- `product/scenario-map.mmd` создается как Mermaid-визуализация к delivery readiness.
- `product/state-model.mmd` создается только если в спецификации есть подтвержденная модель состояний.
...
## Сценарий 22: Delivery readiness pack формируется после валидного финального комплекта
...
- Пользователь явно попросил подготовить readiness pack для команды или согласился на его подготовку

### Пример входа
- "Подготовь пакет, который поможет дизайнеру, frontend, backend и QA начать совместную работу над фичей."

### Ожидаемый маршрут
1. Оркестратор проверяет валидность и сохраненность финального комплекта
2. Оркестратор вызывает `delivery-readiness-agent` с `delivery_readiness_requested: true`
3. Агент формирует `Delivery readiness pack` по `templates/delivery-readiness-template.md`
4. Агент формирует `Scenario map` в Mermaid по `templates/scenario-map-mermaid-template.md`
5. Если в спецификации есть подтвержденные состояния и переходы, агент формирует `State model diagram` в Mermaid по `templates/state-model-mermaid-template.md`
6. Если модель состояний неприменима, агент возвращает `State model diagram: not_applicable` с причиной
7. `artifact-persistence-agent` сохраняет применимые артефакты в `product/`
8. Оркестратор возвращает пользователю ссылки на readiness pack и Mermaid-файлы

### Ожидаемые артефакты
- `product/delivery-readiness-pack.md`
- `product/scenario-map.mmd`
- `product/state-model.mmd`, если применимо
...
### Ожидаемые артефакты
- До исправления `product/delivery-readiness-pack.md` отсутствует или сохранен как `incomplete`
