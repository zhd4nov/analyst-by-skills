...
## Сценарий 13: Пост-пайплайновое интервью финализирует границы без расширения scope
...
- Пользователь готов пройти короткое финализирующее интервью

### Ожидаемый маршрут
1. Оркестратор спрашивает пользователя, готов ли он пройти интервью
2. Оркестратор объясняет цель и тему интервью: уточнение границ без расширения scope
3. Если пользователь согласен: вызывается `scope-finalizer-agent`
4. `artifact-persistence-agent` сохраняет обновленные `Лог уточнений` и связанные артефакты
5. Оркестратор повторно вызывает затронутые профильные скиллы
6. `traceability-auditor-agent` повторно проверяет затронутые артефакты
7. `artifact-persistence-agent` сохраняет `service/traceability-audit.md`
8. `routing-guardian-agent` подтверждает допустимость итоговой финальной агрегации
9. `artifact-persistence-agent` сохраняет `service/routing-decision.md`
10. Оркестратор возвращает обновленный финальный комплект артефактов
...
### Варианты запуска
- Пользователь отказался от интервью `scope-finalizer-agent`
- Интервью `scope-finalizer-agent` было проведено, затронутые артефакты пересобраны, повторные аудит и контроль маршрута завершились успешно
...
2. Оркестратор вызывает `kickoff-briefing-agent` с флагом `after_interview_declined` или `after_scope_finalizer`
3. `kickoff-briefing-agent` формирует `Kickoff brief` по `templates/kickoff-brief-template.md`
4. Если противоречий нет, агент возвращает сигнал `complete`
5. `artifact-persistence-agent` сохраняет `Kickoff brief` в `product/kickoff-brief.md`
6. Оркестратор возвращает пользователю ссылку на brief как дополнительный продуктовый файл, не подменяя финальные артефакты
